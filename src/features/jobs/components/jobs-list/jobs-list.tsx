        ...
        return (
          <>
            ...
            {type === 'dashboard' && (
              <>
                {' '}
                <Link href={`/dashboard/jobs/${id}/update`}>Update</Link>
              </>
            )}
          </>
        );
...
export const JobsList = ({ jobs, isLoading, organizationId, type }: JobsListProps) => {
  const tableColumns = getTableColumns(organizationId, type);

  return (
    <Box data-testid="jobs-list">
      <DataTable isLoading={isLoading || false} data={jobs} columns={tableColumns} />
    </Box>
  );
};
